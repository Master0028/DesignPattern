...
public class MultiViewInFindAndFillAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MultiViewInFindAndFillAdapter(List<HistoryStudy> itemList) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_FIRST_TIME) {
            View view = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.history_study_item_in_findandfill1, parent, false);
            return new FirstTimeViewHolder(view);
        } else {
            View view = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.history_study_item_in_findandfill2, parent, false);
    ...
    class FirstTimeViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(HistoryStudy item) {
            tvTime.setText("Times: " + item.getTimes());
            tvCompletedWords.setText(item.getCompletedWords() + "/" + totalQuestion); // Sử dụng biến totalQuestion của adapter
